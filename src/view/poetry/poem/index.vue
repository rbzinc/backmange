...
import {getPoemData,reqPoemUpdateData,reqPoemAddData,reqPoemDeleteData} from "@/api/modules/poetry.js";
...
const postData = async () => {
  if(poemDates.value.id){
    //发送修改请求
    const result  = await reqPoemUpdateData( poemDates.value.id, poemDates.value.title, poemDates.value.dynasty,poemDates.value. writer, poemDates.value.content, poemDates.value.type);
  }
    //发送增加请求
  else{
    let result  = await reqPoemAddData(poemDates.value.id, poemDates.value.title, poemDates.value.dynasty,poemDates.value. writer, poemDates.value.content, poemDates.value.type);
  }
  if (result.code == 200) {
...
const updateTrademark = (row) => {
  ...
  poemDates.value.writer=row.writer
  poemDates.value.content=row.content
...
const batchDelete = async  (ids) => {
  ...
  // }
  try {
    await reqPoemDeleteData(ids); // 传递实际值
    ElMessage.success('删除成功');
    deleteId.value = [];
    await getPagesDate(); // 刷新数据
  } catch (error) {
...
<template>
  <el-card class="box-card">
    ...
    <el-table border style="margin:10px 0px" :data="attrArr"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="内容" prop="content" label-width="80"></el-table-column>
      <el-table-column label="翻译" prop="translation" label-width="80"></el-table-column>
      <el-table-column label="赏析" prop="shangxi" label-width="80"></el-table-column>
  ...
  <el-dialog v-model="dialogFormVisible" :title="poemDates.id ? '修改品牌' : '添加品牌'" width="840">
